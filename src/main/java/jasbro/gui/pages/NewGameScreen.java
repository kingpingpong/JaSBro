...
public class NewGameScreen extends MyImage {
	...
	public NewGameScreen() {
		...
		slaveList.setModel(new AbstractListModel<CharacterBase>() {
			private List<CharacterBase> characters = filterByTrait(
					Jasbro.getInstance().getSlaveBases(), Trait.FORMERNOBLE, Trait.RARESLAVE, Trait.EXTREMELYRARESLAVE,
					Trait.EXTREMELYRARESLAVE2, Trait.UNSELLABLE);
	...
	/**
	 * Creates a new list using characters as a base, excluding the given traits
	 * @param characters The characters to filter
	 * @param traits The traits to exclude
	 * @return The filtered list
	 */
	private List<CharacterBase> filterByTrait(final List<CharacterBase> characters, final Trait... traits) {
		List<CharacterBase> filteredCharacters = new ArrayList<>(characters.size());
		for(final CharacterBase c : characters) {
	...
	/**
	 * Creates a new list using characters as a base, excluding the given specializations
	 * @param characters The characters to filter
	 * @param specializations The traits to exclude
	 * @return The filtered list
	 */
	private List<CharacterBase> filterBySpecialization(final List<CharacterBase> characters, final SpecializationType... specializations) {
		List<CharacterBase> filteredCharacters = new ArrayList<>(characters.size());
		for(final CharacterBase c : characters) {
			boolean hasSpecialization = false;
			for (final SpecializationType s : specializations) {
