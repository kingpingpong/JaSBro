		...
		for (Customer customer : customers) {
			...
			for (BusinessSecondaryActivity curActivity : secondaryActivities) {
				if (!unwantedActivities.contains(curActivity.getClass())) { //Skip activities the customers does not want to attend
					if(curActivity instanceof Strip){
						if(customer.getType()==CustomerType.BUM || customer.getType()==CustomerType.PEASANT)
							chance=33;
					}
					if(curActivity instanceof Bartend){
						if(customer.getType()==CustomerType.BUM || customer.getType()==CustomerType.PEASANT)
							chance=85;
					}
					if(curActivity instanceof Attend){
						if(customer.getType()==CustomerType.LORD || customer.getType()==CustomerType.CELEBRITY)
							chance=85;
					}
					if(curActivity instanceof BathAttendant){
					...
					if(curActivity instanceof SellFood){
						if(customer.getType()!=CustomerType.BUM)
							chance=75;
					}
					if(curActivity instanceof Offerings){
						if(customer.getType()==CustomerType.SOLDIER || customer.getType()==CustomerType.MERCHANT)
							chance=75;
					}

					
					if (Util.getInt(0, 100) < chance) { // 66% chance that customer wants to participate in an activity type
