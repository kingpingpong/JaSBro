...
public class SellSlaveQuest extends Quest {
	...
	private class SellGirlQuestStage extends QuestStage {
		...
		public String getDescription(Quest quest) {
			String text = TextUtil.t("quests.sellgirlquest.description", getSlave(), getClientType()) + "\n";
			text +=	TextUtil.t("quest.time", getTimeRemaining()) + "\n";
			text += TextUtil.t("quests.sellgirlquest.targetamount", getSlave(), targetAmount) + "\n";
			return text;
		}
		...
		public void handleEvent(MyEvent e, Quest quest) {
			...
			if (e.getType() == EventType.CHARACTERDEATH && e.getSource() == slave) {
				...
				if (getTimeRemaining() <= 0) {
					...
					new MessageScreen(TextUtil.t("quests.sellgirlquest.timeup", getSlave(), 500), 
				...
				if (auction.getSlave() == slave) {
					if (auction.getMaxBid() >= targetAmount) {
						...
						new MessageScreen(TextUtil.t("quests.sellgirlquest.success", getSlave(), reward), 
						...
						new MessageScreen(TextUtil.t("quests.sellgirlquest.failed", getSlave(), auction.getProfit(), penalty), 
								ImageUtil.getInstance().getImageDataByTag(ImageTag.STANDARD, getSlave()), 
								getSlave().getBackground(), true);
