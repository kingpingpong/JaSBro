...
public class CustomQuestStage extends QuestStage implements Serializable {
	private final static Logger log = LogManager.getLogger(CustomQuestStage.class);
	...
	public void modifyActivities(List<ActivityDetails> activityDetails, Time time, List<Charakter> characters,
		...
		if (getTriggers().size() > 0) {
			...
			for (Trigger trigger : getTriggers()) {
				try {
					if (trigger.isTriggered(quest)) {
						WorldEvent worldEvent = Jasbro.getInstance().getWorldEvents()
								.get(triggerToWorldEventMap.get(trigger));
						ActivityDetails activityDetail = new ActivityDetails(ActivityType.CUSTOMEVENT, quest,
								worldEvent.getId(), customize(trigger.getActivityLabel(), quest),
								customize(trigger.getActivityDescription(), quest));
