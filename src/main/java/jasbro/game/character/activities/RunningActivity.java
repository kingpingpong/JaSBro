...
public abstract class RunningActivity {
	private final static Logger log = LogManager.getLogger(RunningActivity.class);
	...
	public void performActivity() {
		...
		if (!abort) {
			...
			if (this instanceof BusinessMainActivity) {
				for (Customer customer : getMainCustomers()) {
					float fameMod = customer.getImportance() * customer.getSatisfaction().getFameModifier() * 10 * fameModifier;
					if (getHouse() != null) {
			...
			if (this instanceof BusinessSecondaryActivity) {
				for (Customer customer : getCustomers()) {            		
					...
					float fameMod = (customer.getImportance() * satisfactionModifiedBy * fameModifier) / 10.0f;
					if (getHouse() != null) {
						getHouse().getFame().modifyFame(fameMod);
					}
					for (Charakter character : getCharacters()) {
