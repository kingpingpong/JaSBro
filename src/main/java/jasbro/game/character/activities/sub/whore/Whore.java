...
public class Whore extends RunningActivity implements BusinessMainActivity {
	...
	public void init() {	
		...
		if(sexType==Sextype.GROUP){
			...
			energyMultiplier=group.getCustomers().size();}
		if(getMainCustomer().getStatus()==CustomerStatus.HORNYSTATUS){energyMultiplier*=1.7f;}
		if(getMainCustomer().getStatus()==CustomerStatus.VERYHORNY){energyMultiplier*=2.4f;}
		if(getMainCustomer().getStatus()==CustomerStatus.STRONGSTATUS){energyMultiplier*=1.3f;}
		if(getMainCustomer().getStatus()==CustomerStatus.LIVELY){energyMultiplier*=1.1f;}
		if(getMainCustomer().getStatus()==CustomerStatus.TIRED){energyMultiplier*=0.7f;}
		if(getMainCustomer().getStatus()==CustomerStatus.SHYSTATUS){energyMultiplier*=0.8f;}
	...
	public List<ModificationData> getStatModifications() {
		...
		if (sexType != null) {
			modificationData.add(new ModificationData(TargetType.ALL, energyMultiplier*1f, sexType));
			modificationData.add(new ModificationData(TargetType.ALL, 0.005f, BaseAttributeTypes.STAMINA));
			modificationData.add(new ModificationData(TargetType.ALL, energyMultiplier*0.3f, SpecializationAttribute.SEDUCTION));
			float obedienceModifier = 0.01f;
			switch (sexType) {
			case GROUP:
				modificationData.add(new ModificationData(TargetType.ALL, -3*energyMultiplier, EssentialAttributes.ENERGY));
			...
			case BONDAGE:
				...
				break;
			default:
				modificationData.add(new ModificationData(TargetType.ALL, -5*energyMultiplier, EssentialAttributes.ENERGY));
			}
	...
	public void perform() {
		...
		if(getMainCustomer().getStatus()==CustomerStatus.HORNYSTATUS){
			messageData.addToMessage("\n" + TextUtil.t("whore.hornycustomer", getCharacter(), getMainCustomer()));;
			setAmountActions(getAmountActions()+1);
			if(sexType==Sextype.GROUP){
				CustomerGroup group=(CustomerGroup)getMainCustomer();
				int payment = getMainCustomer().pay(getCharacter().getMoneyModifier()*group.getCustomers().size()/1.8f);
				modifyIncome(payment);}
				...
				int payment = getMainCustomer().pay(getCharacter().getMoneyModifier());
				modifyIncome(payment);
			}        	getMainCustomer().addToSatisfaction(getMainCustomer().getSatisfactionAmount(), this);

		}
		if(getMainCustomer().getStatus()==CustomerStatus.VERYHORNY){
			messageData.addToMessage("\n" + TextUtil.t("whore.veryhornycustomer", getCharacter(), getMainCustomer()));;
			getMainCustomer().addToSatisfaction(getMainCustomer().getSatisfactionAmount()*2, this);
			setAmountActions(getAmountActions()+2);
			if(sexType==Sextype.GROUP){
				CustomerGroup group=(CustomerGroup)getMainCustomer();
				int payment = getMainCustomer().pay(getCharacter().getMoneyModifier()*group.getCustomers().size()/1.8f);
				modifyIncome(payment*2);}
			else{
				int payment = getMainCustomer().pay(getCharacter().getMoneyModifier());
				modifyIncome(payment*2);
			}
