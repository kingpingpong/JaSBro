...
public class Charakter extends GameObject implements Unit, HasImagesInterface {
	...
	public int getMotivation() {
		return getFinalValue(EssentialAttributes.MOTIVATION);
	}
		...
		for (SpecializationType specializationType : getSpecializations()) {
			...
			for (AttributeType attributeType : specializationType.getAssociatedAttributes()) {
				...
				if (specializationType == SpecializationType.SLAVE || specializationType == SpecializationType.TRAINER) {
					...
					mod = (attribute.getInternValue() / 80.0f) / divideBy + 1;
				}
	...
	public float getDamage() {
		...
		power += getFinalValue(SpecializationAttribute.VETERAN) / 50f;
	...
	public List<Attack> getPossibleAttacks(Battle battle) {
		...
		//Add gained attacks here
		if(getFinalValue(SpecializationAttribute.VETERAN)>10)
			attacks.add( new Attack.StrongStrike(this));
		if(getFinalValue(SpecializationAttribute.MAGIC)>10)
			attacks.add( new Attack.EtherStrike(this));
		if(getFinalValue(SpecializationAttribute.MAGIC)>25)
			attacks.add( new Attack.FireWave(this));
		if(getFinalValue(SpecializationAttribute.MAGIC)>35)
			attacks.add( new Attack.IceStingers(this));
		if(getFinalValue(SpecializationAttribute.MAGIC)>45)
			attacks.add( new Attack.RollingThunder(this));	
		if(getFinalValue(SpecializationAttribute.MAGIC)>60)
			attacks.add( new Attack.StarlightBreaker(this));	
		if(getFinalValue(SpecializationAttribute.MAGIC)>75)
			attacks.add( new Attack.Midnight(this));
		if(getFinalValue(SpecializationAttribute.VETERAN)>30 || getFinalValue(SpecializationAttribute.AGILITY)>20)
			attacks.add( new Attack.SwiftStrike(this));
		if(getFinalValue(SpecializationAttribute.VETERAN)>40 || getFinalValue(SpecializationAttribute.AGILITY)>30)
			attacks.add( new Attack.PreciseStrike(this));
		if(getFinalValue(SpecializationAttribute.SEDUCTION)>25 || getFinalValue(SpecializationAttribute.STRIP)>25)
			attacks.add( new Attack.ButtSmash(this));
		if(getFinalValue(SpecializationAttribute.STRIP)>40)
			attacks.add( new Attack.GracefulKick(this));
		if(getFinalValue(SpecializationAttribute.AGILITY)>45)
			attacks.add( new Attack.ShadowSlicer(this));	
		if(getTraits().contains(Trait.CASTCURE))
	...
	@Override
	public int getDodge() {
		return getIntCalculatedAttribute(CalculatedAttribute.DODGE, 1 + getFinalValue(SpecializationAttribute.AGILITY)/5);
	}
	
	@Override
	public int getSpeed() {
		return getIntCalculatedAttribute(CalculatedAttribute.SPEED, 10 + getFinalValue(SpecializationAttribute.AGILITY)/10);
	}
	...
	public int getPregnancyDurationModifier() {
		int preg = getIntCalculatedAttribute(CalculatedAttribute.PREGNANCYDURATIONMODIFIER, 100); 
		if (this.getTraits().contains(Trait.HEARTOFTHESWARM)) {
			return preg/3;
		} else 
			return preg;
	}
	...
	public void addSpecialization(SpecializationType spec) {
		if (!this.getSpecializations().contains(spec)) {
			if (numberTrees < Jasbro.maxTrees) {
	...
	public List<Warning> getWarnings() {
		...
		if (activity != null) {
			...
			if (getType() == CharacterType.TRAINER) {
				if (activity.getType() == ActivityType.BATHATTENDANT && !getTraits().contains(Trait.LEGACYMASSEUR)) {
					boolean start = false;
					for (Condition condition : getConditions()) {
				...
				if (activity.getType() == ActivityType.BARTEND && !getTraits().contains(Trait.LEGACYBARTENDER)) {
					boolean start = false;
					for (Condition condition : getConditions()) {
						if (condition instanceof StartingAtTheBottom) {
							start = true;
				...
				if (activity.getType() == ActivityType.CLEAN && !getTraits().contains(Trait.LEGACYMAID)) {
					boolean start = false;
					for (Condition condition : getConditions()) {
						if (condition instanceof StartingAtTheBottom) {
							start = true;
				...
				else if ((activity.getType() == ActivityType.SUBMITTOMONSTER || activity.getType() == ActivityType.MONSTERFIGHT) && !getTraits().contains(Trait.LEGACYADVENTURER)) {
					warnings.add(new Warning(Severity.DANGER, TextUtil.t("warnings.catastrophicCommand", this, arguments)));
				}
				else if (activity.getType() == ActivityType.ATTEND && (!getTraits().contains(Trait.LEGACYSTRIPPER) || 
						!getTraits().contains(Trait.LEGACYBARTENDER))) {
					warnings.add(new Warning(Severity.DANGER, TextUtil.t("warnings.catastrophicCommand", this, arguments)));
				}
				else if (activity.getType() == ActivityType.STRIP && !getTraits().contains(Trait.LEGACYSTRIPPER)) {
					warnings.add(new Warning(Severity.DANGER, TextUtil.t("warnings.catastrophicCommand", this, arguments)));
				}
				else if ((activity.getType() == ActivityType.WHORE || activity.getType() == ActivityType.SUBMIT ||
						activity.getType() == ActivityType.TEASE) && !getTraits().contains(Trait.LEGACYWHORE)) {
					warnings.add(new Warning(Severity.DANGER, TextUtil.t("warnings.catastrophicCommand", this, arguments)));
				}
			}
		}
