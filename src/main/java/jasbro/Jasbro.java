 ...
 */
public class Jasbro {
	private final static Logger log = LogManager.getLogger(Jasbro.class);
	...
	public void advanceShift() {
		...
		System.gc();
		if (log.isDebugEnabled()) {
			log.error("Before shift: Current memory(mb): {} Max memory: {}",
	...
	public void advanceDay() {
		...
		//Jasbro.getInstance().save(new File("autosave.xml"));
	}
	...
	public void startNewGame(CharacterBase trainerBase, CharacterBase firstSlave) {
		...
		if(trainer == data.getProtagonist()){
			...
			if(trainer.getAttribute(SpecializationAttribute.EXPERIENCE).getValue() <= 10){
				trainer.getAttribute(SpecializationAttribute.EXPERIENCE).setInternValue(10);
		...
		trainer.getAttribute(EssentialAttributes.MOTIVATION).setInternValue(65);
	...
	public void performEvent(String eventId) {
		if (getWorldEvents().containsKey(eventId)) {
			try {
				...
				log.error("Error while performing event {}", eventId);
				log.throwing(e);
			}
		}
	}
