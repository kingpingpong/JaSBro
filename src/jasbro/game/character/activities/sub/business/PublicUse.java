...
public class PublicUse extends RunningActivity implements BusinessSecondaryActivity {
	...
	public void perform() {
		...
		for (Customer customer : getCustomers()) {
			if(served<slaveMax){
				served++;
				if(Util.getInt(1, 2)==1 && (customer.getStatus()==CustomerStatus.SHYSTATUS || customer.getStatus()==CustomerStatus.DRUNK)){customer.setStatus(CustomerStatus.HAPPY);}
				if(Util.getInt(1, 4)==1){customer.setStatus(CustomerStatus.TIRED);}
		...
		if (served <= slaveMax*2/3) {
				if(getCharacters().size()==1){messageData.addToMessage("\n\n" + TextUtil.t("public.stamina.high.one", getCharacters().get(0)));}
				else{messageData.addToMessage("\n\n" + TextUtil.t("public.stamina.high.more"));}
			}
			else if(served > slaveMax*2/3 && served!=slaveMax){
				if(getCharacters().size()==1){messageData.addToMessage("\n\n" + TextUtil.t("public.stamina.med.one", getCharacters().get(0),arguments));}
				else{messageData.addToMessage("\n\n" + TextUtil.t("public.stamina.med.more"));}
				for(int i=0; i<girls.size(); i++){
					getCharacters().get(i).addCondition(new Buff.RoughenedUp());
				}
			}
			else
			{
				if(getCharacters().size()==1){messageData.addToMessage("\n\n" + TextUtil.t("public.stamina.low.one", getCharacters().get(0), arguments));}
		...
		if(getCharacters().size()==1){
			...
			messageData.addToMessage("\n\n" + TextUtil.t("public.result.tips.one", getCharacters().get(0), arguments));
			
		}
		else{
			messageData.addToMessage("\n\n" + TextUtil.t("public.result.tips.more", arguments));
		}
	...
	public List<ModificationData> getStatModifications() {
		...
		if(getCustomers().size() >= 1){
			...
			if(getCharacters().get(0).getTraits().contains(Trait.PUBLICUSE)){
				modifications.add(new ModificationData(TargetType.ALL, -getCustomers().size()*0.17f/(1+girls.size()), EssentialAttributes.ENERGY));
			}
			else{
