...
public class CharacterFileLoader {
	private final static Logger log = Logger.getLogger(CharacterFileLoader.class);
                    	...
                    	if (files != null) {
	                    	if (Arrays.asList(files).contains("properties.xml")) {
	                    	    try {
	                    	        ...
	                    	        log.error("Error: character could not be loaded: " + file.getName(), e);
	                    	    }
	                    	}
	                    	else if (depth > 0) {
	...
	public CharacterBase loadCharacter(TFile file) {
        ...
        try {
            if (file.exists()) {
                ...
                if (properties != null) {
                    ...
                    tFileInputStream = new TFileInputStream(properties);
        	...
        	log.error("Error on loading character", e);
            throw new MyException("Error on loading character", e);
        }
        finally {
	...
	public synchronized void saveCharacter(CharacterBase character) {
    	InputStream is = null;
        try {
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
            ...
            throw new MyException("Error on writing changes into XML file",e);
        }
        finally {
        	if (is != null) {
        		try {
    ...
    private synchronized void deletePicture(String imageKey) {
        try {
            ...
            throw new MyException("Error on delete", e);
        }
    }
