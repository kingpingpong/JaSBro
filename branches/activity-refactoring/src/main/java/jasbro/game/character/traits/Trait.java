...
public enum Trait implements MinObedienceModifier {
	...
	EXTRAS(true,100, new Perks.Extras()),
	...
	FLIRTY(true,100, new Perks.Flirty()),
	DATASS(true,100, new Perks.DatAss()),
	CATMAID(true,100, new Perks.CatMaid()),
	UNDERTHETABLE(true,100, new Perks.UnderTheTable()),
    ...
    CONTENT(true, 100, new Perks.SimpleTraitEffect(3d, null, CalculatedAttribute.CONTROL)),
    ...
    public int modifyCustomerRating(int initialRating, Customer customer) {
        if (traitEffect != null) {
            return traitEffect.modifyCustomerRating(initialRating, customer);
        }
        else {
