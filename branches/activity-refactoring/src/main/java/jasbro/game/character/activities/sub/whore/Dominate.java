...
public class Dominate extends Whore {
    ...
    public MessageData getBaseMessage() {
    	...
    	Object arguments[] = {getMainCustomer().getName(), getHouse().getName()};
        String message;        
        if (getHouse().getInternName() == null || getHouse().getInternName().trim().equals("")) {
	        message = TextUtil.t("whore.basic1", arguments) + " ";
        }
        else {
        	message = TextUtil.t("whore.basic2", arguments) + " ";
        }
        
        message += TextUtil.t("whore.service", getCharacter(), getMainCustomer());   
        message += "\n";
		
		arguments[0] = getMainCustomer().getName();
    ...
    public void perform() {
    	if (getMainCustomer() != null) {
	    	...
	    	modifyIncome(pay);
	    	Object arguments[] = {getMainCustomer().getSatisfaction().getText(), pay};
	    	getMessages().get(0).addToMessage("\n\n" + TextUtil.t("whore.end", getCharacter(),
	    	        getMainCustomer(), arguments));   
    	}
    }
