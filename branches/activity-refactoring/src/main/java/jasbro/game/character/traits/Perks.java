...
import jasbro.game.character.conditions.Buff.AttributeToAmount;
...
public class Perks {
	...
	public static class WetForYou extends TraitEffect {
		@Override
		public int modifyCustomerRating(int rating, Customer customer) {
	...
	public static class Submissive extends TraitEffect {
		...
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Whore && Util.getInt(0, 100)<30) {
					Whore whoreActivity = (Whore) activity;
				...
				//Same for monsters
				if (activity instanceof SubmitToMonster && Util.getInt(0, 100)>50) {
		...
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITYCREATED) {
				...
				if (activity instanceof Bartend) {
					...
					bartend.setBonus(bartend.getBonus()+character.getFinalValue(BaseAttributeTypes.INTELLIGENCE));
				}
			}
		}
	} 
	...
	//Fondling may happen. Each fondle nets some fame. Adds flavor text.
	public static class DatAss extends TraitEffect{
		@Override
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Bartend) {
					int occurence=0;
					float chance=(character.getFinalValue(BaseAttributeTypes.CHARISMA)+character.getFinalValue(BaseAttributeTypes.OBEDIENCE));
					for (Customer customer : activity.getCustomers()) {
						if(Util.getInt(0, 100)<chance){
							customer.addToSatisfaction(character.getFinalValue(BaseAttributeTypes.CHARISMA), activity);
							character.getFame().modifyFame(1);
							occurence++;
							chance--;
						}
					}
					if(occurence>100){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("DATASS.hundred",character));}
					else if(occurence>50){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("DATASS.fifty",character));}
					else if(occurence>30){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("DATASS.thirty",character));}
					else if(occurence>20){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("DATASS.twenty",character));}
					else if(occurence>0){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("DATASS.some",character));}
					if(occurence!=0){activity.getAttributeModifications().add(new AttributeModification(occurence*0.001f, BaseAttributeTypes.OBEDIENCE, character));}
	...
	//Same, with blowjobs.
	public static class UnderTheTable extends TraitEffect{
		@Override
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Bartend) {
					int occurence=0;
					int pay=0;
					float chance=3+Util.getInt(0, 10)+(character.getFinalValue(BaseAttributeTypes.OBEDIENCE)+character.getFinalValue(Sextype.ORAL)/20);
					for (Customer customer : activity.getCustomers()) {
						if(Util.getInt(0, 100)<chance & occurence<character.getFinalValue(Sextype.ORAL)/5){
							customer.addToSatisfaction(character.getFinalValue(Sextype.ORAL)/3, activity);
							character.getFame().modifyFame(1);
							occurence++;
							chance--;
							pay+=character.getFinalValue(Sextype.ORAL)/3;
							customer.payFixed(character.getFinalValue(Sextype.ORAL)/3);
							activity.modifyIncome(pay);
						}
					}
					Object arguments[]={occurence,pay};
					if(occurence!=0){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("UNDERTHETABLE.oral",character,arguments));
					activity.getAttributeModifications().add(new AttributeModification(occurence*0.05f, Sextype.ORAL, character));}
	...
	//Same, with les sexual harassment
	public static class Flirty extends TraitEffect{
		@Override
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Bartend) {
					int occurence=0;
					float chance=(character.getFinalValue(BaseAttributeTypes.CHARISMA)+character.getFinalValue(BaseAttributeTypes.INTELLIGENCE));
					for (Customer customer : activity.getCustomers()) {
						if(Util.getInt(0, 100)<chance){
							customer.addToSatisfaction(character.getFinalValue(BaseAttributeTypes.CHARISMA)/2, activity);
							character.getFame().modifyFame(2);
							occurence++;
							chance--;
						}
					}
					if(occurence>30){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("FLIRTY.thirty",character));}
					else if(occurence>20){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("FLIRTY.twenty",character));}
					else if(occurence>10){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("FLIRTY.ten",character));}
					else if(occurence>0){activity.getMessages().get(0).addToMessage("\n"+TextUtil.t("FLIRTY.some",character));}
					if(occurence!=0){activity.getAttributeModifications().add(new AttributeModification(occurence*0.001f, BaseAttributeTypes.CHARISMA, character));}
				}
			}
		}
	}  	
	...
	public static class TargetAudience extends TraitEffect{
		...
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Strip && Util.getInt(1, 5)==4) {
	...
	//A customer may be chosen at random. Stuff happens based on his preferences
	public static class Extras extends TraitEffect{
		@Override
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				RunningActivity activity = (RunningActivity) e.getSource();
				if (activity instanceof Strip) {
					Customer lucky = null;
					int chance = character.getFinalValue(BaseAttributeTypes.CHARISMA)/40 +character.getFinalValue(SpecializationAttribute.STRIP)/100;
					for (Customer customer : activity.getCustomers())
					{
						if(Util.getInt(0, 250)<chance){lucky=customer;}						
					}
					if(lucky!=null && lucky.getType()!=CustomerType.BUM)
					{

						int tips = (int)(lucky.getMoney() / (800.0 / character.getFinalValue(SpecializationAttribute.STRIP)) + Util.getInt(10, 20));
						activity.modifyIncome(tips);
						Object arguments[] = {tips, lucky.getName()};
						activity.getMessages().get(0).addToMessage("\n\n" +TextUtil.t("STRIP.extras",character,arguments) +"\n");
						if(lucky.getPreferredSextype()==Sextype.ANAL)
						{
							if(lucky.getGender()==Gender.MALE)
							{ 
								lucky.addToSatisfaction(character.getFinalValue(Sextype.ANAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.anal.male",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.ANAL, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.ANAL, character));
							}
							else
							{
								lucky.addToSatisfaction(character.getFinalValue(Sextype.ANAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.anal.female",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.LESBIAN, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.ANAL, character));
							}
						}
						else if(lucky.getPreferredSextype()==Sextype.VAGINAL)
						{
							if(lucky.getGender()==Gender.MALE)
							{ 
								lucky.addToSatisfaction(character.getFinalValue(Sextype.VAGINAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.vaginal.male",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.VAGINAL, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.VAGINAL, character));
							}
							else
							{
								lucky.addToSatisfaction(character.getFinalValue(Sextype.VAGINAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.vaginal.female",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.LESBIAN, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.VAGINAL, character));
							}
						}

						else if(lucky.getPreferredSextype()==Sextype.ORAL)
						{
							if(lucky.getGender()==Gender.MALE)
							{ 
								lucky.addToSatisfaction(character.getFinalValue(Sextype.ORAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.oral.male",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.ORAL, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.ORAL, character));
							}
							else
							{
								lucky.addToSatisfaction(character.getFinalValue(Sextype.ORAL), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.oral.female",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.TRIBADISM, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.ORAL, character));
							}
						}
						else if(lucky.getPreferredSextype()==Sextype.FOREPLAY)
						{
							if(lucky.getGender()==Gender.MALE)
							{ 
								lucky.addToSatisfaction(character.getFinalValue(Sextype.FOREPLAY), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.foreplay.male",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.LAPDANCE, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.FOREPLAY, character));
							}
							else
							{
								lucky.addToSatisfaction(character.getFinalValue(Sextype.FOREPLAY), activity);
								activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.foreplay.female",character));
								activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.LAPDANCE, character));
								activity.getAttributeModifications().add(new AttributeModification(1.0f, Sextype.FOREPLAY, character));
							}
						}
						...
						{
							//She found a weirdo.
							activity.getMessages().get(0).addToMessage(TextUtil.t("STRIP.extras.orgy",character));
							activity.getMessages().get(0).addImage(ImageUtil.getInstance().getImageDataByTag(ImageTag.GROUP, character));
							activity.getAttributeModifications().add(new AttributeModification(activity.getCustomers().size()*0.05f, Sextype.GROUP, character));
							activity.getAttributeModifications().add(new AttributeModification(-15.0f, EssentialAttributes.ENERGY, character));
							character.getFame().modifyFame(activity.getCustomers().size());
						}
					}
				}
			}
	...
	public static class TheUntouchable extends TraitEffect{
		...
		public void handleEvent(MyEvent event, Charakter character, Trait trait) {
			if (event.getType() == EventType.CUSTOMERSARRIVE) {
				CustomersArriveEvent customerEvent = (CustomersArriveEvent) event;
				for (Customer customer : customerEvent.getCustomers()) {
					if(Util.getInt(1, 10)==5){customer.setInitialMoney((int) (customer.getInitialMoney()*2));}
					if(Util.getInt(1, 100)==5){customer.setInitialMoney((int) (customer.getInitialMoney()*5));}
				}
			}
		}
	}
	...
	public static class CompulsiveCleaner extends TraitEffect {
		...
		public void handleEvent(MyEvent e, Charakter character, Trait trait) {
			if (e.getType() == EventType.ACTIVITY) {
				...
				if (house != null) {
					house.modDirt(5-character.getFinalValue(SpecializationAttribute.CLEANING)/54);
				}
			}
		}
	}
	...
	public static class KeepEmComing extends TraitEffect{
		@Override
		public double getAttributeModified(CalculatedAttribute calculatedAttribute, double currentValue, Charakter character) {
			if (calculatedAttribute == CalculatedAttribute.AMOUNTCUSTOMERSPERSHIFT && Util.getInt(1, 5)==5) {
				if(currentValue>=10){
				currentValue+=Util.getInt(4, 8);
			}
			}
			return currentValue;
	...
	public static class Renowed extends TraitEffect{
		...
		public void handleEvent(MyEvent event, Charakter character, Trait trait) {
			if (event.getType() == EventType.CUSTOMERSARRIVE) {
				...
				for (Customer customer : customerEvent.getCustomers()) {
					if(customer.getType()==CustomerType.CELEBRITY){
						customer.setInitialMoney((int) (customer.getInitialMoney()*1.50));
					}
				}
					...
					for (Charakter curCharacter : activity.getCharacters()) {
						for (Condition condition : curCharacter.getConditions()) {
							if (condition instanceof Buff) {
								...
								for (AttributeToAmount attributeToAmount : buff.getAttributeModifiers()) {
									if (attributeToAmount.getAttribute() instanceof BaseAttributeTypes) {
										attributeToAmount.setAmount(attributeToAmount.getAmount() + 1);
									}
									else {
										attributeToAmount.setAmount(attributeToAmount.getAmount() + 10);
									}
								}
							}
						}
