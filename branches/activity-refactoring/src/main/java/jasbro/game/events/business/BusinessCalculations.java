...
public class BusinessCalculations {
    ...
    public void assignCustomers(List<BusinessActivityExecutor> mainActivities, List<Customer> customers, List<Customer> remainingCustomers) {
    	int bestValue;
    	do {
    		BusinessActivityExecutor activity = null;
	    	Customer customer = null;
    		bestValue = 0;
	    	for (int i = 0; i < mainActivities.size(); i++) {
		    	for (int j = 0; j < customers.size(); j++) {
		    		BusinessActivityExecutor curActivity = mainActivities.get(i);
			    	...
			    	for (Charakter character : activity.getCharacters()) {
			    	    for (Trait trait : character.getTraits()) {
			    	        curValue = trait.modifyCustomerRating(curValue, curCustomer);
			    	    }
			    	}
			    	}
		...
		for (Customer customer : customers) {
			...
			for (BusinessSecondaryActivity curActivity : secondaryActivities) {
				if (!unwantedActivities.contains(curActivity.getClass())) { //Skip activities the customers does not want to attend
					int baseChance=10;
					if(customer.getPreferedSecondaryActivity1() == curActivity.getType()
							|| customer.getPreferedSecondaryActivity2() == curActivity.getType()){
						baseChance+=30;
					}
